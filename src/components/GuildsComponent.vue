...
export default {
  ...
  methods: {
    setGuild(guildIdx) {
      userStore.currentGuild = guildIdx
      userStore.currentGuildId = this.guilds[guildIdx].id
    },
  },
  beforeMount() {
    if (this.userStore.ifAuthenticated) this.userStore.setGuilds(this.token as string);
  },
};
</script>
  ...
  >
    ...
    <div v-if="ifAuthenticated">
      <img
        ...
        :class="currentGuild === guild ? 'active' : ''"
